...
// console.log(reactive(props.meeting.meetingId));
    ...
    <router-link :to="`/meeting/${props.meeting.meetingId}`">
      ...
      <div class="meeting__body">
        ...
        <div class="meeting__body-detail">
          <button class="status status__rated" v-if="props.meeting.evaluated">
            평가완료
          </button>
          <button
            @click.prevent="onClick($event)"
            class="status status__rate"
            v-else-if="props.meeting.canRate"
          >
            평가하기
          </button>
          <div class="status status__on" v-else-if="!props.meeting.closed">
            모집중
          </div>
          <div class="status status__off" v-else-if="props.meeting.closed">
            모집완료
          </div>
