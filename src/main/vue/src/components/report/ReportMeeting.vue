...
const selectInput = reactive({
    ...
    items : [
                {id : 1, name :'사행성 글'},
                {id : 2, name :'부적절한 게시글 제목/내용'},
                {id : 3, name :'판매권유 및 상행위 글'},
                {id : 4, name :'기타'}
            ],
    hasError : false,
    isOpened : false,
...
function report(reportedMeetingId,meetingReportType,meetingReportReason){
   if(meetingReportType==0){
    ...
    const response = api.report.reportMeeting(reportedMeetingId,meetingReportType,meetingReportReason);
    response
        .then((data)=>data.json())
        .then((result)=>{
            if(result)
   ...
   <div id="modal-report-member" class="modal-wrapper z-idx-1 modal-report">
      <div class="modal">
        ...
        <div class="input-text">
            ...
            <textarea  id="member-input__content--id" placeholder="상세 내용을 적어주세요. (최대 100자)" class="input-text__content" v-model="meetingReportReason"></textarea>
        </div>
        <div class="btn-wrapper">
            ...
            <span id="report-btn" data-modal="#modal-report-member" class="comment__btn btn btn-round member__report-btn" @click.prevent="report(reportedMeetingId,meetingReportType,meetingReportReason)">신고하기</span>
        </div>
    </div>
</div>
</div>
