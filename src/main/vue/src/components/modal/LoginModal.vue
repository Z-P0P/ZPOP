            ...
            <div class="modal__body">
                ...
                <div class="login-btn-container">
                    <a class="login-btn login-btn--kakao text-hidden" id="kakao-login-btn"
                        @click.prevent="onKakaoLoginClick"
                        >카카오
                        로그인 버튼</a>
...
const redirectUrl = encodeURI(`${import.meta.env.VITE_REDIRECT}?login=naver`);
...
function onKakaoLoginClick() {
    Kakao.Auth.authorize({redirectUri: `${import.meta.env.VITE_REDIRECT}?login=kakao`})
}
</script>
